            ...
            foreach (PolMesh mesh in polFile.Meshes)
            {
                foreach (PolTexture texture in mesh.Textures)
                {
                    foreach (var textureName in texture.TextureNames)
                    {
                        ...
                        if (_materialFactory is ToonMaterialFactory)
                        {
                            // No need to load pre-baked shadow texture if material
                            // used is Toon material, since shadow texture will be
                            // generated by shader.
                            // Note: all shadow texture name starts with "^"
                            texture2D = textureName.StartsWith("^") ? null : textureProvider.GetTexture(textureName);
                        }
                        else
                        {
